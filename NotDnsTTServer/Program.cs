...
namespace NotDnsTTServer
{
    internal class Program
    {
                                ...
                                if (DnsMessage.Header.QueryType == Ae.Dns.Protocol.Enums.DnsQueryType.A)
                                {
                                    ...
                                    lock (thislock)
                                    {
                                        ...
                                        if (!conns.ContainsKey(connid))
                                        {
                                            ...
                                            new Thread(() =>
                                            {
                                                Program.tcphandlerthread(connid);
                                            }).Start();
                                        }
                                        new Thread(() =>
                                        {
        ...
        static void tcphandlerthread(int connid)
        {
            try
            {
        ...
        public static IPEndPoint dstendpoint;
        public static IPEndPoint dnsendpoint;
        static void Main(string[] args)
        {
            if (args.Length < 5)
            {
                Console.WriteLine("Usage: DnsMessengerServer <serveraddress> <base64_key> <src_domain_address> <dst_domain_address> <destination_address>");
                Console.WriteLine("Press any key to exit...");
                Console.ReadKey();
                return;
            }
            var udpserver = new UDPSocket();
            udpserver.Server("0.0.0.0", 5300);
            DnsMessengerEncryption.Encryption.key = Convert.FromBase64String(args[1]);
            srcdomainaddress = args[2];
            dstdomainaddress = args[3];
            dstendpoint = IPEndPoint.Parse(args[4]);
